...
## [1.9.1] - 2026-09-22 - Recruitment Mode Screen Render Fix
- **「勧誘モードで画面が表示されないよ」画面表示バグの修正**:
  - `StoryModeScreen.tsx` において、ヘッダーに配置した仲間追加アイコン `UserPlus` のインポートが不足していたため、実行時に `ReferenceError: UserPlus is not defined` が発生し、勧誘モード画面が白画面・非表示となっていた不具合を解消。
  - `lucide-react` より `UserPlus` を正常にインポートし、全章（第1章〜隠し第9章）の勧誘モード画面が正常に表示されることを確認。
  - トップヘッダーの「勧誘モード」ボタンおよび各画面の戻るボタン（「討伐モードへ戻る」）の導線を点検・同期。
- **テストの拡充**:
  - `test/runner.ts` のシナリオ7に `StoryModeScreen` の完全レンダリング検証を追加し、コンポーネントの参照エラーや表示不具合を未然に防止するテストガードを確立（全56テストパス）。
