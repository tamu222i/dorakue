...
### 会得呼吸の確認機能＆未取得呼吸のシークレット数量・解放条件表示（宿モード・編成・図鑑統合）
- **会得した呼吸の型・血鬼術の全情報確認機能**:
  - 各隊士が修得している技名（ルビ付き・ふりがな）、肩書（『水の呼吸 壱ノ型』『ヒノカミ神楽』等）、消費BP、威力、対象範囲、詳細説明文を一覧表示。
  - 最強奥義（例: 『日暈の龍 頭舞い』『生々流転』『火雷神』『凪』等）には「★ 最強奥義 ★」の黄金バッジと専用スタイリングを付与。
  - 基礎技（0BP通常斬撃等）も整理して確認可能。
- **未取得呼吸のシークレット（秘匿）数量＆解放条件の可視化**:
  - ネタバレ防止のため、未修得の技名は `🔒【未解禁】？？？？？？`（最終奥義は `🔒【極限奥義】？？？？？？`）でシークレット表示。
  - 呼吸全体の数量把握（例: `全集中・呼吸の型 会得進捗: 1 / 7 型 (14%)`、`未解禁シークレット: 残り 6 型`）とプログレスバーを配置。
  - 各シークレット技に `Lv.〇 で開眼`（最終奥義は `Lv.〇 で極限奥義が開眼`）の成長条件目安を表示。
- **宿屋（休養・編成・勧誘）および大図鑑への統合**:
  - **部隊編成（`PartyFormationModal`）**: 出撃枠および控え隊士カードに「呼吸確認 (〇/〇型)」ボタンを配置。タップで「全集中・呼吸の型 修練帳」モーダル（`BreathingDetailModal`）を展開。
  - **隊士勧誘（`InnScreen` スカウト）**: 宿に集う隊士・柱を仲間に迎える前に、現在修得している技と将来の覚醒ポテンシャルを確認可能。
  - **座敷休養（`InnScreen`）**: 休息中の隊士アイコンをタップしてそのまま修練帳を開いて確認可能。
  - **300種大図鑑（`ZukanScreen`）**: 遭遇済みの隊士・鬼のプロファイル内に `BreathingProgressView` を直接統合。鬼殺隊で育成中の隊士はリアルタイムのレベルと修得進捗が即座に反映。
- **TDD/BDDテスト追加**:
  - `test/runner.ts` に「Scenario 15: Inspecting learned breathing techniques and unacquired secret quantity in Inn & Zukan」を追加（全149件のテストをクリア）。

### 戦闘勝利時のレベルアップ都度表示＆新呼吸開眼時の派手な演出モーダル（VictoryLevelUpModal）
- **戦闘勝利時のレベルアップ都度ポップアップ通知**:
  - 戦闘勝利時にレベルアップした隊士がいる場合、専用モーダル (`VictoryLevelUpModal`) が都度開く設計へ刷新。
  - 以前のレベル ➔ 新レベル、全ステータス（最大HP、最大BP、攻撃力、防御力、素早さ）の成長量（+▲）、次レベルまでの必要経験値を明示。
  - 複数人が同時にレベルアップした際のスムーズなページ送り（次へ／閉じる）に対応。
- **新呼吸・新技開眼時の専用ファンファーレ＆派手な演出**:
  - レベル到達により新しい呼吸・技を開眼した際は、専用ファンファーレ（気流音＋金管アルペジオ）、属性別オーラグラデーション、脈動グロー、漢字紋章スタンプ（『水』『炎』『雷』『日』等）、スポットライトカードによる派手な演出で祝福。
  - 最強奥義の場合は「★ 最強奥義 ★」「【極限奥義会得！！】」の特別演出。
- **TDD/BDDテスト追加**:
  - `test/runner.ts` に「Scenario 14: Post-battle level up and flashy new breathing technique acquisition modal」を追加。

### アイテムの効力・自動BP回復の不具合修正＆呼吸技選択連動の完全化
- **「アイテムの効力自動BP回復が効かない」不具合の完全解消**:
  - **技選択画面での自動補給連携**:
    - 従来は隊士の現在BPが技コストに足りない場合、ボタンが非活性（`disabled`）となり技を選択できなかったため、所持しているBP回復アイテム（特製おにぎり等）の自動回復が発動するコードパスへ到達できない構造的課題を解消。
    - インベントリ内のBP回復可能量（`AutoItemService.getTotalRecoverableBp`）をリアルタイムに計算し、不足分をアイテムで補填できる場合は技ボタンを活性化。
    - 技一覧に `🍙 おにぎり自動回復` バッジおよび `自動回復可` のステータスを表示。
    - タップ時に即座に自動回復が発動し、回復効果音・回復ログ・回復数値演出（`+25BP`）とステータスバーのリアルタイム更新を実施。
  - **戦闘中の自動BP回復トリガーの多重強化**:
    - ラウンド開始時: 生存隊士のBPが低下（最大BPの35%以下または15以下）している場合、自動で特製おにぎりを食して全集中の呼吸を整えBP回復。
    - 技行動実行時: 発動に必要なBPに達するまで複数アイテムを自動消費して奥義・強力呼吸技を確実に発動。
    - 技発動後: 技消費でBPが危険域に達した際にも即時自動回復を適用。
    - AUTO戦闘モード: インベントリ内の回復可能BPを算入して自動で上位呼吸技をスムーズに連続使用。
  - **戦闘中道具画面からの手動使用対応**:
    - 常時全自動適用に加え、道具画面から現在の隊士へ直接アイテム（傷薬・おにぎり等）を使用できる手動操作ボタンを追加。
  - **TDD/BDDテスト追加**:
    - `test/runner.ts` に「Scenario 13: Automatic BP recovery via inventory items」を追加（全114件のテストをクリア）。


### モブ鬼の強化＆ボスとの格差是正・「2〜3戦でレベルアップ」成長曲線への最適化
- **モブ鬼の耐久力・攻撃力強化（ボスとの極端な格差を解消）**:
  - モブ鬼が瞬殺されてボスとの格差が開きすぎていた問題を是正。
  - HP・攻撃力・防御力を強化（Lv.1でHP約100、Lv.10でHP約620、Lv.21でHP約1300、Lv.45でHP約2900）。
  - 味方パーティの呼吸技で2〜3ターンかけて戦う歯ごたえのあるバトルへ調整し、ボス戦への自然な架け橋となるよう再設計。
  - 高レベルのモブ鬼に強力な連携技『血鬼爪・連撃』を追加。
- **2〜3戦で確実にレベルアップするテンポ設計**:
  - レベルに応じた必要経験値式を `PartyAggregate.calculateNextExp(level) = level * 200 + 220` に刷新。
  - 勝利時の獲得経験値もモブLv×40+20ベースへ引き上げ、**「同格・適正の野良鬼に2〜3回勝てば確実にレベルアップ」**する黄金バランスを確立。
  - 討伐成功時のログに「つぎのレベルまで あと 〇〇 経験値（あと1〜2勝でLv.UP!）」の進捗案内を追加。
  - 修業場ヘッダーに「🔥 約2〜3戦でLv.UP！」の案内バッジを表示。
